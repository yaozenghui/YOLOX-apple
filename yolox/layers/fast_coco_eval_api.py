...
class COCOeval_opt(COCOeval):
    def accumulate(self, p = None):
        ...
        r_tp = -np.ones((T,K,A,M))
        r_fp = -np.ones((T,K,A,M))
        r_fn = -np.ones((T,K,A,M))
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    gts = np.logical_and(               gtm,  np.logical_not(gtIg) )

                    tp_sum = np.cumsum(tps, axis=1).astype(dtype=np.float)
                    fp_sum = np.cumsum(fps, axis=1).astype(dtype=np.float)
                    for t, (tp, fp, gt) in enumerate(zip(tp_sum, fp_sum, gts)):
                        tp = np.array(tp)
                        fp = np.array(fp)
                        nd = len(tp)
                        rc = tp / npig
                        ...
                        r_tp[t,k,a,m] = tp[-1]
                        r_fp[t,k,a,m] = fp[-1]
                        r_fn[t,k,a,m] = len(gt) - r_tp[t,k,a,m]
...
class COCOeval_opt_fast(COCOeval):
    """
