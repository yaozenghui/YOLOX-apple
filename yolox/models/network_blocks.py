...
class BottleneckMHSA1(nn.Module):
    # Standard bottleneck
    def __init__(
        self,
        in_channels,
        ...
        Conv = DWConv if depthwise else BaseConv
        self.conv1 = BaseConv(in_channels, hidden_channels, 1, stride=1, act=act)
        self.mhsa = MHSA(hidden_channels, 13, 13)
        self.conv2 = Conv(hidden_channels, out_channels, 3, stride=1, act=act)
        self.use_add = shortcut and in_channels == out_channels
...
class BottleneckMHSA2(nn.Module):
    # Standard bottleneck
    def __init__(
        self,
        in_channels,
        ...
        Conv = DWConv if depthwise else BaseConv
        self.conv1 = BaseConv(in_channels, hidden_channels, 1, stride=1, act=act)
        self.mhsa = MHSA(hidden_channels, 26, 26)
        self.conv2 = Conv(hidden_channels, out_channels, 3, stride=1, act=act)
        self.use_add = shortcut and in_channels == out_channels
...
class CSPLayerMHSA1(nn.Module):
    ...
    def __init__(
        ...
        module_list = [
            BottleneckMHSA1(
                hidden_channels, hidden_channels, shortcut, 1.0, depthwise, act=act
            )for _ in range(n)]+[eca_layer(k_size=3)]
        self.m = nn.Sequential(*module_list)
...
class AttentionCSPLayer(nn.Module):
    ...
    def __init__(
        ...
        module_list = [
            Bottleneck(
                hidden_channels, hidden_channels, shortcut, 1.0, depthwise, act=act
            )for _ in range(n)]+[CoordAtt(inp=hidden_channels,oup=hidden_channels)]
        self.m = nn.Sequential(*module_list)
...
class BoT3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5, e2=1, w=13, h=13):  # ch_in, ch_out, number, , expansion,w,h
...
class ELAN(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.cv4(x2)
        x4 = self.cv5(x3)
        x_cat = torch.cat((x1, x1, x3, x4),dim=1)
        x_out = self.cv6(x_cat)
        return x_out
